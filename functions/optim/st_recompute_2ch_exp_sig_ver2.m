function [comp_ws, rsp, conv_nb] = st_recompute_2ch_exp_sig_ver2(x,stim,data,Xv,Yv,keep,hrf,normStimRF)
...
% 
% maxNorm = cellfun(@max, nbS) / cellfun(@max, nbT);
% norm_nbT = cellfun(@(x) x*maxNorm,nbT,'uni',false);
% 
...
rsp = cellfun(@(P, W) P .* repmat(W, size(P, 1), 1), conv_nb, repmat({comp_ws'}, nruns, 1), 'uni', false);
